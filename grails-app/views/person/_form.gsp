...
<g:each in="${personInstance?.transactions?}" var="t">
    <li><g:link controller="txn" action="show" id="${t.id}">${t?.encodeAsHTML()}</g:link></li>
</g:each>
<li class="add">
<g:link controller="txn" action="create" params="['person.id': personInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'txn.label', default: 'Txn')])}</g:link>
</li>
</ul>
